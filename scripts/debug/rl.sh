...
SEQ_LEN=$(( ((PROMPT_LEN + SEP_SIZE - 1) / SEP_SIZE) * SEP_SIZE ))
GBS=${GBS:-64}
...
ACTOR_RECOMPUTE_LEN=$((SEQ_LEN + RL_MAX_NEW_TOKENS - 1))
if [ $((ACTOR_RECOMPUTE_LEN % SEP_SIZE)) -ne 0 ]; then
  PADDED_ACTOR_RECOMPUTE_LEN=$(( ((ACTOR_RECOMPUTE_LEN + SEP_SIZE - 1) / SEP_SIZE) * SEP_SIZE ))
  RL_MAX_NEW_TOKENS=$((PADDED_ACTOR_RECOMPUTE_LEN - SEQ_LEN + 1))
  ACTOR_RECOMPUTE_LEN=$PADDED_ACTOR_RECOMPUTE_LEN
fi
...
echo "[rl-debug] PROMPT_LEN=$PROMPT_LEN SEQ_LEN=$SEQ_LEN RL_MAX_NEW_TOKENS=$RL_MAX_NEW_TOKENS ACTOR_RECOMPUTE_LEN=$ACTOR_RECOMPUTE_LEN SEP_SIZE=$SEP_SIZE"
